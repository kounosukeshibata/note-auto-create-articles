...
| 状況 | 推奨エージェント |
|---|---|
| 新機能の設計相談 → 実装の前 | `design-architect` |
| 設計が固まった後の実装 | `tdd-implementation-runner` |
| 既存コードが壊れた | `bug-investigator` |
| インフラ複数ステップ作業 | `gcloud-infra-ops` |
| 単発 gcloud コマンド | MCP `gcloud` 直接 |
